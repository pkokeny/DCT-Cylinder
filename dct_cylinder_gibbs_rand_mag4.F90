...
type(C_PTR) :: plan
integer :: dimxy,dimz,radius,cropf,lor, greens, s
integer :: partsp, npartFOVxy, npartFOVz, crop, field_only, manual_sum, npartFOV
...
!magscale(:)= (/ 0.4966D0, 0.3460D0, 0.2372D0, 0.1664D0, 0.1243D0, 0.1143D0, 0.1000D0 /)
!magscaleZ(:)= (/ 0.5090D0, 0.4114D0, 0.3325D0, 0.2687D0, 0.2172D0, 0.1755D0, 0.1419D0, &
!0.1147D0, 0.0927D0, 0.0749D0 /) !0.29ppm HeXie bulk
...
magscaleXY(:)= (/ 0.35D0, 0.21D0, 0.15D0, 0.14D0, 0.11D0, 0.09D0, 0.11D0, 0.09D0, 0.11D0, &
0.11D0 /) !1.11ppm HeXie bulk perpendicular
...
C=A+(B*B)
!A-Greens Function
A=((2.0D0*(B*B)-A)/SQRT(C*C*C*C*C))/(4.0D0*pi)
B=0.0D0
!Put zero in middle
...
open(unit = 1, file = "Greens_k_perp_FOV101030_156c.dat", form="unformatted")
write(1) C
close(1)
...
!LATTICE
!do p = 1,partsp
    !Xps(p)=-1*(partsp/2+1)+p
    !Yps(p)=-1*(partsp/2+1)+p
    !Zps(p)=-1*(partsp/2+1)+p
!    Xps(p)=-1*(9)+p
!    Yps(p)=-1*(9)+p
!    Zps(p)=-1*(9)+p
!end do
...
open (unit = 1, file = "perp_values.txt")
...
open(unit = 1, file = "Magnitude_vf12x1r5p2197_rand_101030.dat", form="unformatted")
write(1) B
close(1)
...
!open(unit = 1, file = "Field_vf13x1r3p1728_rand3D.dat", form="unformatted")
!write(1) field_test
!close(1) 
                              
open(unit = 1, file = "Field_vf12x1r5p2197_rand_101030.dat", form="unformatted")
write(1) A
close(1) 
...
D=(A*A)+(B*B)
C=field*((dX*vf)/2.0d0)*radius**2*(((B*B)-(A*A))/(D*D));
A=0.0d0
B=0.0d0

where (D<radius**2) C=((-1.0d0/6.0d0)*(dX*vf)*field)
D=0.0d0
...
open(unit = 1, file = "Field_vf12x1r5p2197_cont_101030.dat", form="unformatted")
read(1) B
close(1)

!B=B*(1.11/0.56)

!field_test(:,:,:)=B(:,:,1:cropf)
!open(unit = 1, file = "Field_vf21x03r3p1728_cont_rand3D.dat", form="unformatted")
!write(1) field_test
!close(1) 
...
!open(unit = 1, file = "Magnitude_vf21x0403r3p1728_rand3D_flip.dat", form="unformatted")
!write(1) field_test
!close(1) 
B=0.0d0
!else
...
FFTW_REDFT00, FFTW_REDFT00, FFTW_REDFT00,FFTW_ESTIMATE)
    ...
    WRITE(filenameCk,'("Complex_k_gibbsMATCH",I1,"_vf12x1r5p2197_rand",I2,".dat")') s, t
